...
private:
	...
	uint64_t hash_value;
public:
	CMSketch(long long _w, long long _d) { //w is the size of the hash area, d is the number of hash functions in the Mediean trick
	...
	{
		long long min_value = sp_negative?MAX_CNT_CO:MAX_CNT_CO_P;
		sead_c temp;
		...
		for (int i = 0; i < d; i++)
		{
			...
			min_value = min(min_value,predict(temp,gamma,sp_negative));
		}
