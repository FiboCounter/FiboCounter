...
char insert[100000000 + 100000000 / 5][15];

const int new_number = sign_flag ? 1024 * 16 / COUNTER_SIZE : 1024 * 16 / valid_COUNTER_SIZE;   // = 1024 * (sead 的位数/ 新算法的位数)

// three experiment functions: CAIDA_experiment, webpage_experiment and synthetic_experiment.
void CAIDA_experiment(long long number, double &ret_error, int version, double n, long long width, long double &sigma, int sketch, double &ret_aae, long double &aae_sig, string file_name)
{
	...
	if (sketch == 4)
	{
		for (int i = 0; i < LOOP_NUM; ++i)
		{
			//s[i] = new CMSketch(n * new_number / 2 / m, m);
		}
	...
	if (sketch == 5)
	{
		for (int i = 0; i < LOOP_NUM; ++i)
		{
			//s[i] = new CUSketch(n * new_number / 2 / m, m);
		}
	...
	if (sketch == 6)
	{
		for (int i = 0; i < LOOP_NUM; ++i)
		{
			//s[i] = new CSketch(n * new_number / 2 / m, m);
		}
	...
	sprintf(filename, "../dataset/webdocs_form01.dat");
	//  
	...
	for (int m = 0; m < LOOP_NUM; ++m)
	{
		...
		switch(sketch){
			case 0: s[m] = new CMSketch(n * 1024 / 4 / width, width); break;
			case 1: s[m] = new CSketch(n * 1024 / 4 / width, width); break;
			case 2: s[m] = new CSketch(n * 1024 / 4 / width, width); break;
			case 4: s[m] = new CMSketch(n * new_number / 2 / width, width); break;
			case 5: s[m] = new CUSketch(n * new_number / 2 / width, width); break;
			case 6: s[m] = new CSketch(n * new_number / 2 / width, width); break;
			default : return ;
		}
		...
		while (fread(insert[package_num], package_size, 1, file_stream))
		{
			string s(insert[package_num++], package_size);
			fread(insert[package_num], 4, 1, file_stream);
			unmp[m][s]++;
			total_size++;
			if(total_size == number) break;
		}
	...
	file << n << "  " << ret_error << "  " << sigma << "  " << ret_aae << "  " << aae_sig << endl;
